  ...
  resizeImg (imageB64) {
    return new Promise((resolve, reject) => {
      ...
      img.onload = function () {
        ...
        const resize_width = this.width > max_width ? max_width : this.width
        const scaleFactor = resize_width / this.width
        const resize_height = this.height * scaleFactor
    ...
    customFields.forEach((field) => {
      ...
      if (
        ...
        if (locale && Array.isArray(field.text)) {
          ...
          const textLocale = field.text.find((e) => e.locale === locale)
